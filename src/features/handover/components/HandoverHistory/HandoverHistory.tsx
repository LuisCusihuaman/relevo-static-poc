...
export function HandoverHistory({ onClose, patientData, hideHeader = false }: HandoverHistoryProps) {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      case 'guarded': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      case 'unstable': return 'bg-orange-100 text-orange-800 border-orange-200';
    ...
    <div className="h-full flex flex-col bg-white">
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 bg-blue-50 border-b border-gray-200">
          <h4 className="font-medium text-sm text-gray-900 mb-2">Patient Timeline</h4>
          ...
          <div className="p-4 space-y-4">
            {handoverHistory.map((handover, index) => (
              <Card 
                ...
                onClick={() => setSelectedHandover(selectedHandover === handover.id ? null : handover.id)}
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <Calendar className="w-4 h-4 text-gray-500" />
                      <span className="text-sm font-medium">{handover.date}</span>
                    </div>
                    <Badge className={getStatusColor(handover.status)}>
                      {handover.status === 'in-progress' ? 'Current' : 'Completed'}
                    </Badge>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Clock className="w-3 h-3 text-gray-400" />
                    <span className="text-xs text-gray-600">{handover.time}</span>
                    <ArrowRight className="w-3 h-3 text-gray-400" />
                    <span className="text-xs text-gray-600">{handover.shift}</span>
                  </div>
                </CardHeader>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2 text-xs">
                        ...
                        <span className="text-gray-600">From: {handover.primaryPhysician}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-xs">
                        <ArrowRight className="w-3 h-3 text-gray-400" />
                        <span className="text-gray-600">To: {handover.receivingPhysician}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <ul className="text-xs text-gray-700 space-y-1">
                        ...
                        {handover.keyPoints.length > 2 && (
                          <li className="text-gray-500">+{handover.keyPoints.length - 2} more...</li>
                        )}
                      </ul>
                    </div>
                    ...
                    {selectedHandover === handover.id && (
                      <div className="pt-3 border-t border-gray-200 space-y-3">
                        ...
                        <div>
                          <span className="text-xs text-gray-600 font-medium">All Key Points:</span>
