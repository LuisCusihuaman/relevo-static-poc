...
const getServiceAlerts = (
  t: TFunction<"notificationsView", undefined>,
  patientsData: Patient[],
): ServiceAlert[] => [
  {
    id: 1,
    type: "infection_control",
    service: t("services.infection_control"),
    title: t("alerts.newMDRO.title"),
    message: t("alerts.newMDRO.message", {
      patientName:
        patientsData.find((p) => p.id === 1)?.name || "Maria Rodriguez",
      room: patientsData.find((p) => p.id === 1)?.room || "PICU-01",
    }),
    details: t("alerts.newMDRO.details"),
    priority: "HIGH",
    patientId: 1,
    patientName:
      patientsData.find((p) => p.id === 1)?.name || "Maria Rodriguez",
    ...
    actionRequired: true,
    actions: [
      t("actions.implementContactPrecautions"),
      t("actions.notifyAttendingPhysician"),
      t("actions.updateCarePlan"),
    ],
  },
  {
    id: 2,
    type: "pharmacy",
    service: t("services.pharmacy"),
    title: t("alerts.drugAllergy.title"),
    message: t("alerts.drugAllergy.message", {
      patientName:
        patientsData.find((p) => p.id === 2)?.name || "Carlos Gonzalez",
      room: patientsData.find((p) => p.id === 2)?.room || "PICU-03",
    }),
    details: t("alerts.drugAllergy.details"),
    priority: "HIGH",
    patientId: 2,
    patientName:
      patientsData.find((p) => p.id === 2)?.name || "Carlos Gonzalez",
    ...
    actionRequired: true,
    actions: [
      t("actions.reviewMedicationOrders"),
      t("actions.contactPrescribingPhysician"),
      t("actions.considerAlternativeTherapy"),
    ],
  },
  {
    id: 3,
    type: "laboratory",
    service: t("services.laboratory"),
    title: t("alerts.criticalLabResult.title"),
    message: t("alerts.criticalLabResult.message", {
      patientName: patientsData.find((p) => p.id === 3)?.name || "Ana Silva",
      room: patientsData.find((p) => p.id === 3)?.room || "PICU-05",
    }),
    details: t("alerts.criticalLabResult.details"),
    priority: "HIGH",
    patientId: 3,
    patientName: patientsData.find((p) => p.id === 3)?.name || "Ana Silva",
    room: patientsData.find((p) => p.id === 3)?.room || "PICU-05",
    ...
    actions: [
      t("actions.adjustInsulinProtocol"),
      t("actions.checkKetones"),
      t("actions.notifyEndocrinology"),
    ],
  },
  {
    id: 4,
    type: "nutrition",
    service: t("services.nutrition"),
    title: t("alerts.dietModification.title"),
    message: t("alerts.dietModification.message", {
      patientName: patientsData.find((p) => p.id === 4)?.name || "David Kim",
      room: patientsData.find((p) => p.id === 4)?.room || "PICU-07",
    }),
    details: t("alerts.dietModification.details"),
    priority: "MED",
    patientId: 4,
    patientName: patientsData.find((p) => p.id === 4)?.name || "David Kim",
    room: patientsData.find((p) => p.id === 4)?.room || "PICU-07",
    ...
    actionRequired: true,
    actions: [
      t("actions.updateDietOrders"),
      t("actions.notifyNursingStaff"),
      t("actions.patientFamilyEducation"),
    ],
  },
  {
    id: 5,
    type: "social_work",
    service: t("services.social_work"),
    title: t("alerts.dischargePlanning.title"),
    message: t("alerts.dischargePlanning.message"),
    details: t("alerts.dischargePlanning.details"),
    priority: "MED",
    ...
    actionRequired: true,
    actions: [
      t("actions.coordinateEquipment"),
      t("actions.scheduleFamilyTraining"),
      t("actions.confirmInsuranceCoverage"),
    ],
  },
  {
    id: 6,
    type: "quality_safety",
    service: t("services.quality_safety"),
    title: t("alerts.fallRisk.title"),
    message: t("alerts.fallRisk.message"),
    details: t("alerts.fallRisk.details"),
    priority: "INFO",
    ...
    actionRequired: false,
    actions: [
      t("actions.completeAssessments"),
      t("actions.updateSafetyPlans"),
      t("actions.documentChanges"),
    ],
  },
];
...
export function NotificationsView({
  ...
  const { t } = useTranslation("notificationsView");
  ...
  const serviceAlerts: ServiceAlert[] = getServiceAlerts(t, patientsData);
    ...
    <div className={`bg-background ${isMobile ? "" : "h-full"}`}>
      ...
      <div className={`${isMobile ? "px-4" : "px-6"} mb-4`}>
            ...
            <>
              <Select value={filterService} onValueChange={setFilterService}>
                <SelectTrigger className="w-56">
                  <SelectValue placeholder={t("filters.servicePlaceholder")} />
                </SelectTrigger>
                <SelectContent>
              ...
              <Select value={filterPriority} onValueChange={setFilterPriority}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="INFO">{t("priorities.info")}</SelectItem>
                </SelectContent>
              </Select>
            </>
          )}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {showAcknowledged ? t("filters.hide") : t("filters.show")}{" "}
            {t("filters.acknowledged")}
          </Button>
        </div>
      </div>
      ...
      <div className={`${isMobile ? "px-4" : "px-6"} space-y-3 pb-6`}>
        {filteredAlerts.length === 0 ? (
          <Card className="glass-card rounded-2xl p-8 text-center">
            ...
            <p className="text-muted-foreground">
              {searchTerm
                ? t("noAlerts.adjustCriteria")
                : t("noAlerts.allAcknowledged")}
            </p>
          </Card>
        ) : (
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <Badge
                              ...
                              className="text-xs"
                            >
                              {t(`priorities.${alert.priority.toLowerCase()}`)}
                            </Badge>
