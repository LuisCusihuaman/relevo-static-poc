...
import { Bell, AlertTriangle, Clock, CheckCircle, X, Filter, Search, Calendar, Info } from 'lucide-react';
...
import { patients } from '../data/patients.store';
import { type Patient } from '../data/types';

interface ServiceAlert {
  id: number;
  type: 'infection_control' | 'pharmacy' | 'laboratory' | 'nutrition' | 'social_work' | 'quality_safety';
  service: string;
  title: string;
  message: string;
  details: string;
...
export function NotificationsView({ patients: patientsProp }: NotificationsViewProps = {}) {
  ...
  const serviceAlerts: ServiceAlert[] = [
    {
      ...
      details: 'Multi-drug resistant organism detected in respiratory culture. Contact isolation required. Staff must use gown and gloves.',
      priority: 'HIGH',
      patientId: 1,
      patientName: patientsData.find(p => p.id === 1)?.name || 'Maria Rodriguez',
    ...
    },
    {
      ...
      details: 'ACE inhibitor contraindicated with current diuretic regimen. Recommend alternative medication or dose adjustment.',
      priority: 'HIGH',
      patientId: 2,
      patientName: patientsData.find(p => p.id === 2)?.name || 'Carlos Gonzalez',
    ...
    },
    {
      ...
      details: 'Immediate intervention required. Consider adjusting insulin drip rate and monitoring ketones.',
      priority: 'HIGH',
      patientId: 3,
    ...
    },
    {
      ...
      details: 'Scheduled for bronchoscopy tomorrow at 10:00 AM. NPO after midnight tonight.',
      priority: 'MED',
      patientId: 4,
    ...
    },
    {
      ...
      message: 'Patient Emma Johnson (PICU-09) - Home oxygen equipment needs to be arranged.',
      details: 'Family requires training on home oxygen use and equipment maintenance before discharge.',
      priority: 'MED',
    ...
    },
    {
      ...
      message: 'Multiple patients require fall risk reassessment after medication changes.',
      details: 'New sedation protocols may affect mobility scores. Please reassess and update safety measures.',
      priority: 'INFO',
  ...
  const filteredAlerts = serviceAlerts
    .filter(alert => {
      ...
      if (searchTerm && !alert.title.toLowerCase().includes(searchTerm.toLowerCase()) && 
          !alert.patientName?.toLowerCase().includes(searchTerm.toLowerCase())) return false;
      return true;
    })
    .sort((a, b) => {
  ...
  const stats = {
    ...
    actionRequired: serviceAlerts.filter(a => a.actionRequired && !a.acknowledged).length
  };
  ...
  return (
    <div className={`bg-background ${isMobile ? '' : 'h-full'}`}>
          ...
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-muted-foreground">Alerts from hospital services requiring attention</p>
                <div className="flex items-center gap-4 mt-2 text-sm">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-destructive rounded-full animate-pulse"></div>
                    <span className="text-destructive font-medium">{stats.unacknowledged} unacknowledged</span>
                  </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={() => serviceAlerts.forEach(alert => acknowledgeAlert(alert.id))}>
                  Acknowledge All
                </Button>
              </div>
            </div>
            ...
            {!isMobile && (
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="bg-chart-1/10 rounded-xl p-3 text-center">
                  <div className="text-2xl font-semibold text-chart-1">{stats.unacknowledged}</div>
                  <div className="text-sm text-muted-foreground">Unacknowledged</div>
                </div>
                <div className="bg-primary/10 rounded-xl p-3 text-center">
                  <div className="text-2xl font-semibold text-primary">{stats.actionRequired}</div>
      ...
      <div className={`${isMobile ? 'px-4' : 'px-6'} mb-4`}>
        <div className={`flex gap-4 ${isMobile ? 'flex-col' : 'flex-row'} mb-3`}>
      ...
      <div className={`${isMobile ? 'px-4' : 'px-6'} space-y-3 pb-6`}>
        {filteredAlerts.length === 0 ? (
          <Card className="glass-card rounded-2xl p-8 text-center">
            ...
            <h3 className="font-medium text-foreground mb-2">No alerts found</h3>
            <p className="text-muted-foreground">
              {searchTerm ? 'Try adjusting your search criteria' : 'All alerts have been acknowledged'}
            </p>
          </Card>
        ) : (
              ...
              >
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    {/* Service Icon */}
                    <div className={`p-3 rounded-lg border ${getServiceColor(alert.type)}`}>
                      <span className="text-2xl">{getServiceIcon(alert.type)}</span>
                    </div>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <Badge variant={alert.priority === 'HIGH' ? 'destructive' : alert.priority === 'MED' ? 'default' : 'secondary'} className="text-xs">
                              {alert.priority}
                            </Badge>
                          ...
                          <div className="mb-2">
                            <p className="text-sm font-medium text-muted-foreground">{alert.service}</p>
                          </div>
                  ...
                  {/* Action Buttons */}
                  {alert.actionRequired && !alert.acknowledged && (
                    <div className={`flex gap-2 mt-4 pt-3 border-t border-border ${isMobile ? 'flex-col' : 'flex-row'}`}>
                      {alert.actions.slice(0, isMobile ? 2 : 3).map((action, index) => (
                        <Button
                          key={index}
