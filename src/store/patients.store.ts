...
export const formatDiagnosis = (diagnosis: Diagnosis | string): string => {
...
export const patients: Patient[] = [
  {
    ...
    status: "in-progress", // CLEANED: Simple status, not handover-specific
  ...
  },
  {
    ...
    diagnosis: {
      ...
      secondary: ["Cardiac catheterization"],
    },
  ...
  },
  {
    ...
    ipassEntries: [
      ...
      },
      {
        ...
        type: "action_list",
        timestamp: new Date(Date.now() - 1800000),
        author: "Dr. Roberto Martinez",
        isComplete: true,
      },
      {
        id: "ip-006",
        type: "situation_awareness",
  ...
  },
  {
    ...
    diagnosis: {
      ...
      secondary: ["New onset seizures", "EEG monitoring"],
    },
  ...
  },
  {
    ...
    // Medical with proper Alert objects
    diagnosis: {
      primary: "Post-surgical recovery",
      secondary: ["Appendectomy", "Pain management"],
    },
  ...
  },
  {
    ...
    diagnosis: {
      ...
      secondary: ["Allergic asthma", "Environmental triggers"],
    },
    ...
    handoverHistory: [
      {
        ...
        from: "Dr. Night Shift",
        to: patient.team?.attending || "Unknown",
        type: "incoming",
        status: "completed",
      },
      {
        ...
        from: patient.team?.attending || "Unknown",
        to: patient.team?.residents?.[0] || "Dr. Day Resident",
        type: "outgoing",
        status: "pending",
      },
    ],
    recentActivity: [
      {
        ...
        action: "Note Added",
        author:
          patient.lastIPassUpdate?.author ||
          patient.team?.residents?.[0] ||
          "Dr. Resident",
        details: "Patient summary updated with latest lab results.",
        section: "patient_summary",
      },
      {
        id: `ra-${patient.id}-2`,
        ...
        ? [
            {
              ...
              content:
                "Patient showing gradual improvement in respiratory status. Decreased work of breathing noted. FiO2 weaned from 60% to 45% overnight. Family remains anxious but engaged in care discussions.",
              author: {
                name: "Dr. Sarah Chen",
  ...
  return [...patients].sort((a, b) => {
    switch (sortBy) {
      case "severity": {
        ...
        return (
          severityOrder[b.illnessSeverity] - severityOrder[a.illnessSeverity]
        );
      }
...
export const getPatientStats = () => {
  ...
  const totalAlerts = getAlertCount(allAlerts);
  const criticalAlerts = getCriticalAlertCount(allAlerts);
  ...
  return {
    ...
    totalAlerts,
    criticalAlerts,
    totalNotes: patients.reduce(
      (sum, patient) => sum + (patient.ipassEntries?.length || 0),
      0,
    ),
  };
};
...
export const mockDetailedAlerts: Record<
  number,
  Array<{
    id: string;
  ...
  1: [
    {
      id: "alert-1-1",
      type: "infection",
      message: "Germen Multi Resistente - OXA48/163",
      priority: "high",
    },
    {
      id: "alert-1-2",
      type: "allergy",
      message: "Alergia al Látex",
      priority: "medium",
    },
  ],
  2: [
    {
      id: "alert-2-1",
      type: "medication",
      message: "Reacción a Vancomicina",
      priority: "medium",
    },
  ],
  3: [
    {
      id: "alert-3-1",
      type: "critical",
      message: "Paciente Anticoagulado",
      priority: "high",
    },
  ],
};
...
export const dailySetupPatients = patients.map((patient) => ({
  ...
  age: patient.age,
  room: patient.room,
  diagnosis: formatDiagnosis(patient.diagnosis),
...
export const searchPatients = (query: string) => {
  ...
  return patients.filter(
    (p) =>
      ...
      p.room.toLowerCase().includes(lowerQuery) ||
      p.mrn?.toLowerCase().includes(lowerQuery) ||
      formatDiagnosis(p.diagnosis).toLowerCase().includes(lowerQuery) ||
      p.team?.attending.toLowerCase().includes(lowerQuery),
  );
};
